...
contract SponserPaymaster is BasePaymaster {
    ...
    function _validatePaymasterUserOp(
        ...
        (bytes32 pmNonce, uint256 expiry, bytes memory pmSignature) = abi
            .decode(paymasterData, (bytes32, uint256, bytes));
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked("PAYMASTER_APPROVAL:", userOpHash, pmNonce, expiry)
        );
        bytes32 ethSignedHash = messageHash.toEthSignedMessage(); // converts to standard ETH signed msg
        address recoveredAddr = ethSignedHash.recover(ethSignedHash);
        /// @custom:check Check if the recovered addr from signature hash is the paymaster
        if (recoveredAddr != paymasterSigner)
            revert SponsorPaymaster__InvalidSignature();
        ...
        if (maxCost > maxGasCostPerOperation)
            revert SponserPaymaster__ExceedsMaxGasCost(
                maxCost,
                maxGasCostPerOperation
            );
        ...
        if (newTotal > maxSponsorshipPerUser) {
            revert SponserPaymaster__ExceedsUserSponsorLimit(
                smartAccount,
                newTotal,
    ...
    function _postOp(
        ...
        (address smartAccount, bytes32 pmNonce, ) = abi.encode(
            context,
